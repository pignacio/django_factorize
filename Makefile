...
lint:
	...
	-pep8 django_factorize tests
	-find django_factorize -name "*.py" -exec pylint {} +
...
test-cover: test-deps clean-pyc
	python setup.py nosetests --with-coverage --cover-package=django_factorize
...
coverage: test-deps clean-pyc
	coverage run --source django_factorize setup.py nosetests
	make coverage-show
...
docs: docs-deps
	rm -f docs/django_factorize.rst
	rm -f docs/modules.rst
	sphinx-apidoc -o docs/ django_factorize
	$(MAKE) -C docs clean
	$(MAKE) -C docs html
...
yapf:
	find django_factorize -name "*.py" -exec echo {} \; -exec yapf --diff {} \;
